...
function renderMetrics(data) {
  ...
  const budget = `${settings.daily_request_budget ?? 8} req / ${(settings.daily_input_token_budget ?? 16000).toLocaleString()} tok`;
  const rows = [
  ...
  else { pill.textContent = "运行正常"; pill.className = "pill"; }
  const alert = $("alert");
...
function renderEntries() {
  ...
  $("entries").innerHTML = filtered.length ? filtered.map((row) => `<article class="entry-card"><div class="entry-card-head"><strong>${escapeHtml(row.scope_type)} · ${escapeHtml(row.scope_key || "*")}</strong><span class="tag">${escapeHtml(labels[row.status] || row.status)}</span></div><p class="entry-content">${escapeHtml(row.content)}</p><div class="entry-meta"><span>${escapeHtml(row.kind)}</span><span>${escapeHtml(row.trust_level)}</span><span>v${escapeHtml(row.version)}</span>${triggersOf(row).length ? `<span>${escapeHtml(triggersOf(row).join(" / "))}</span>` : ""}</div><div class="entry-actions"><button class="minor entry-rollback" data-id="${escapeHtml(row.entry_id)}" data-version="${escapeHtml(row.version)}">回滚版本</button><button class="minor entry-edit" data-id="${escapeHtml(row.entry_id)}">编辑</button></div></article>`).join("") : '<div class="empty">没有匹配的记忆条目.</div>';
}
...
function openEntry(row = null) {
  ...
  if (row) {
    ...
    form.status.value = row.status === "archived" ? "draft" : row.status;
...
function openRollback(row) {
  ...
  form.reset();
  form.entry_id.value = row.entry_id;
  form.version.value = Math.max(1, Number(row.version) - 1);
...
$("schedule-form").addEventListener("submit", async (event) => {
  event.preventDefault();
  try { await api("POST", "schedules", { ...Object.fromEntries(new FormData(event.target)), enabled: true }); showToast("学习时间点已添加"); await load(); } catch (error) { showToast(String(error), true); }
});
...
$("schedules").addEventListener("click", async (event) => {
  ...
  if (!button) return;
  try { await api("POST", `schedules/${button.dataset.id}`, { enabled: button.dataset.enabled === "1" }); showToast("学习时间点状态已更新"); await load(); } catch (error) { showToast(String(error), true); }
});
