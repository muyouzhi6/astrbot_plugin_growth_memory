...
主人通过 `growth_memory_note` 明确提交的 `behavior_rule`、`profile_fact` 或 `milestone` 会**跳过审核流程，立即生效**：

```
用户: "记住，以后画图不要偏黄"
Bot: （由对话 LLM 按当前人格自然回应）
```

- 工具成功时返回空结果，由对话 LLM 自然组织回复，不注入固定确认话术
- 适用于 `owner` / `task` / `global` 层级的 `behavior_rule`、`profile_fact` 和 `milestone`
- 信任级别自动设为 `owner_explicit`
- 写入后立即刷新内存快照，下一轮对话生效
- 重复内容直接忽略，内容扩展自动版本化更新；模糊冲突进入维护队列
- 已有 `manual` 条目不会被自动工具改写，只能在 Dashboard 中确认维护结果

### 定期记忆维护

学习时间点结束 1 小时后，插件会在后台处理维护队列并分析同作用域的相似自动条目：

- LLM 只在冷路径中决定 `replace`、`merge` 或 `ignore`
- 相似条目合并时保留版本和审计记录，重复的相似决策不会反复调用模型
- Dashboard 的“记忆维护”区域支持查看对比、保留现有、采用新内容、重试和立即维护
- 没有配置 Reviewer Provider 时，维护会延后，不会假装已经处理
