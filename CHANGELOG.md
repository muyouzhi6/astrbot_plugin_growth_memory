...
### 维护候选召回
- 放宽相似记忆候选发现，结合 CJK/ASCII 词片段重叠和字符顺序相似度，避免不同措辞的同一偏好直接新增条目。
- 候选发现只负责把可能相关的条目交给 LLM；`merge`、`replace`、`keep_both` 和 `ignore` 仍全部由 LLM 决策。
- 新增低表面相似重复条目的即时入队和定期维护回归测试。

## v0.6.0 (2026-08-10)

### 核心改进
- 主人提交 `behavior_rule`、`profile_fact` 或 `milestone` 时立即写入 active 条目并刷新运行态快照，不再依赖提交文本中的触发词。
- 成功的记忆工具调用返回空结果，由对话 LLM 自然回应，移除固定的确认话术。
- 相同内容直接忽略；正文扩展自动版本化更新；模糊冲突进入维护队列。已有 `manual` 条目不会被自动工具直接覆盖。

### 定期维护
- 新增 `MaintenancePipeline`，在每个学习时间点后 1 小时处理冲突队列，并分析同作用域的相似自动条目。
- 维护 LLM 支持 `replace`、`merge`、`ignore`；维护结果保留 entry version 和 audit log，重复的相似决策不会重复调用模型。
- 没有 Reviewer Provider 时保留待处理状态并延后，不伪造成功；失败项支持有限次数自动重试和 Dashboard 手动重试。
- 新增 `maintenance_queue`、`maintenance_runs` 存储和旧库字段迁移；重启时会恢复中断中的维护工作。

### Dashboard
- 新增维护队列对比视图、维护运行历史、立即维护、保留现有内容、采用新内容和重试操作。

## v0.4.3 (2026-08-10)

- **默认 kind 改为 behavior_rule**: 强意图表达（"记住以后/永远/不要再"）现在默认作为行为规则处理，触发快速通道立即生效.
- **优化 kind 参数说明**: 工具描述中明确各类型适用场景，引导模型正确选择 behavior_rule（立即生效）或 profile_fact（审核流程）.
- **典型场景标注 kind**: Few-shot 示例中标注推荐的 kind 参数值.

## v0.4.2 (2026-08-10)

- **优化工具描述优先级**: `growth_memory_note` 添加【优先使用】标签和 Few-shot 示例，明确用于长期偏好、习惯、风格要求，避免与短期事实型记忆工具冲突.
- 典型场景示例："记住以后拍照要女友视角"、"我喜欢简洁的设计风格"、"以后回复要简短" 等强意图表达.
- 明确不用于短期事实（生日、电话、地址、账号）、一次性指令、通用知识、敏感信息.

## v0.4.1 (2026-08-10)

- **主人即时指令快速通道**: 主人使用强意图词（"以后/永远/记住/不要再/必须/禁止"）的 `behavior_rule` 跳过审核，立即生效，下一轮对话即可使用.
- **情感记忆支持**: 新增 `emotional_bond` 条目类型，支持 `expires_in_days` 参数设置过期天数（0 = 永久，> 0 = N 天后自动归档）.
- 每 5 分钟自动清理过期条目，过期后立即从运行态快照移除，不影响聊天性能.
- 快速通道自动检测冲突键去重，防止主人重复添加相同规则.
- 兼容 `context_aware` 插件：短期上下文信号可触发 `growth_memory_note` 写入 7 天过期的 `emotional_bond`.
- 所有测试通过（65 项），代码格式检查通过.
